...
```sql
alter tablename
-- db_data_type是数据库的数据类型，比如int(1)，varchar(64)
...
数据检索SQL一般由select子句、from子句、join子句、where子句、group by子句、having子句 、order by子句组成。from子句后的其他子句不是必须要有的，根据实际需要进行使用。各个子句出现的相对顺序不能颠倒。Select检索数据的`逻辑处理流程`为：
